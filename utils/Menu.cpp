...
bool Menu::run()
{
    while (true)
    {
        system(CLEAR);
        int i = 1;
        std::cout << std::setw(menuName.length() * 1.6) << menuName << std::endl;

        std::cout << std::string(menuName.length() * 2, '=') << std::endl;
        for (auto &item : menuItems)
        {
            std::cout << i << ". " << item.str << std::endl;
            i++;
        }
        ...
        if (choice > 0 && choice <= menuItems.size())
        {
            ...
            else
            {
                ...
                else if (chosenItemString == "read from file")
                {
                    ...
                    {
                        std::cout << "Not implemented yet1" << std::endl;
                        waitForUser();
                    }
                }
                else if (chosenItemString == "generate graph")
                {
                    std::cout << "Not implemented yet2" << std::endl;
                    waitForUser();
                }
                else if (chosenItemString == "first algorithm")
                {
                    std::cout << "Not implemented yet5" << std::endl;
                    waitForUser();
                }
                else if (chosenItemString == "second algorithm")
                {
                    std::cout << "Not implemented yet6" << std::endl;
                    waitForUser();
                }
                else if (chosenItemString == "display matrix")
                {
                    std::cout << "Not implemented yet3" << std::endl;
                    waitForUser();
                }
                else if (chosenItemString == "display list")
                {
                    std::cout << "Not implemented yet4" << std::endl;
                    waitForUser();
                }
                else
                {
